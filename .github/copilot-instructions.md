...
This is the **ioBroker Snips adapter** (`iobroker.snips`). Snips was an offline, privacy-focused speech recognition and natural language understanding platform. The adapter integrates Snips hardware/software with ioBroker via MQTT, enabling voice-controlled smart home automation without cloud dependency. Note: Snips was discontinued in February 2020 when Sonos acquired the company, but existing installations continue to function.

## Adapter-Specific Context
- **Adapter Name**: iobroker.snips
- **Primary Function**: Offline speech-to-text and natural language understanding via Snips platform, communicating over MQTT
- **Key Dependencies**: `mqtt` npm package for MQTT broker communication, `@iobroker/adapter-core` for adapter base
- **Configuration Requirements**: MQTT broker URL (default: localhost), port (default: 1883), client ID, topic filter, language setting
- **Integration**: Requires the `text2command` ioBroker adapter to execute voice commands
- **Architecture**: Runs as a daemon, subscribes to Snips MQTT topics, processes intents, controls ioBroker states
...
[CUSTOMIZE: The Snips adapter communicates with Snips hardware via MQTT. When working on MQTT-related code, ensure proper connection handling with reconnect logic, QoS settings appropriate for the message type, and graceful shutdown that unsubscribes from topics and ends the client connection cleanly. The adapter processes Snips NLU intents (published to `hermes/intent/#` topics) and publishes TTS responses (to `hermes/tts/say`). When the MQTT broker is unavailable, the adapter should log an appropriate error and set the `info.connection` state to false.]
